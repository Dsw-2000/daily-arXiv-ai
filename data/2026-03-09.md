...
TL;DR: SKETCHGAIT introduces a new sketch-based visual modality for gait recognition that captures dense part-level structural cues without semantic labels, complementing existing silhouette- and parsing-based approaches through a disentangled multi-modal framework.


<details>
  <summary>Details</summary>
Motivation: Existing gait recognition methods rely on silhouette- or parsing-based representations with limitations: silhouettes are sparse and miss internal structural details, while parsing methods depend heavily on upstream human parsers with unstable performance across datasets and sometimes inferior results to silhouettes.

Method: The authors identify an underexplored paradigm in the design space of edge density and supervision form: dense part-level structure without explicit semantic labels. They introduce SKETCH as a new visual modality that extracts high-frequency structural cues (limb articulations, self-occlusion contours) directly from RGB images via edge-based detectors in a label-free manner. They propose SKETCHGAIT, a hierarchically disentangled multi-modal framework with two independent streams for modality-specific learning and a lightweight early-stage fusion branch to capture structural complementarity between label-guided parsing and label-free sketch.

Result: Extensive experiments on SUSTech1K and CCPG datasets validate the proposed modality and framework: SketchGait achieves 92.9% Rank-1 accuracy on SUSTech1K and 93.1% mean Rank-1 accuracy on CCPG.

Conclusion: The paper demonstrates that sketch-based representations provide a valuable new modality for gait recognition, offering dense structural cues without semantic labels that complement existing approaches. The proposed disentangled multi-modal framework effectively leverages the structural complementarity between label-guided parsing and label-free sketch, achieving state-of-the-art performance on benchmark datasets.
...
TL;DR: Thinking with Spatial Code converts RGB videos into explicit 3D representations with oriented bounding boxes and semantic labels, enabling LLMs to reason over spatial variables for physical-world visual question answering.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models lack explicit 3D spatial understanding for physical-world reasoning, limiting their ability to answer questions requiring geometric and perspective-aware inference.

Method: Proposes a spatial encoder that unifies 6D object parsing and tracking with geometric prediction to transform RGB videos into structured spatial code with 3D bounding boxes and semantic labels, then fine-tunes LLMs using reinforcement learning with a spatial rubric reward.

Result: Outperforms proprietary vision-language models on VSI-Bench, achieving state-of-the-art performance in physical-world visual question answering.

Conclusion: Explicit 3D spatial representations enable LLMs to perform geometrically grounded reasoning, advancing physical-world visual question answering capabilities.
...
TL;DR: First coupled robustness verification framework for heatmap-based keypoint detectors that bounds joint deviation across all keypoints using MILP formulation, providing sound guarantees.


<details>
  <summary>Details</summary>
Motivation: Keypoint detection is fundamental to many vision tasks but neural models remain vulnerable to small perturbations. Formal robustness verification for keypoint detectors is largely unexplored due to high-dimensional inputs and continuous coordinate outputs, with prior approaches being decoupled and conservative.

Method: Proposes a coupled verification framework that formulates robustness as a falsification problem using mixed-integer linear programming (MILP). Combines reachable heatmap sets with a polytope encoding joint deviation constraints across all keypoints. Infeasibility certifies robustness while feasibility provides counterexamples.

Result: The method achieves high verified rates and remains effective under strict error thresholds where decoupled methods fail. The approach is proven to be sound: if it certifies the model as robust, then the keypoint detection model is guaranteed to be robust.

Conclusion: First coupled robustness verification framework for keypoint detectors that captures interdependencies between keypoints and provides formal guarantees, outperforming decoupled approaches especially under strict error thresholds.
...
TL;DR: DreamCAD is a multi-modal generative framework that produces editable BReps from point-level supervision without CAD-specific annotations, using parametric patches and differentiable tessellation, achieving SOTA performance on CAD benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing CAD generative methods are limited by small annotated datasets with explicit design histories or BRep labels, while millions of unannotated 3D meshes remain untapped, hindering scalable CAD generation.

Method: DreamCAD represents BReps as sets of parametric patches (e.g., Bézier surfaces) and uses differentiable tessellation to generate meshes, enabling large-scale training on 3D datasets while reconstructing connected, editable surfaces. Also introduces CADCap-1M dataset with 1M+ GPT-5 generated descriptions.
...
Conclusion: DreamCAD enables scalable CAD generation without CAD-specific annotations by leveraging point-level supervision and parametric patch representations, with the CADCap-1M dataset advancing text-to-CAD research.
...
TL;DR: ABRA is an adversarial batch representation augmentation method that addresses biological batch effects in cell painting images by modeling batch-wise statistical fluctuations as structured uncertainties and synthesizing worst-case perturbations to improve domain generalization.
...
Method: Frames bio-batch mitigation as Domain Generalization problem. Proposes Adversarial Batch Representation Augmentation (ABRA) that parameterizes feature statistics as structured uncertainties to model batch-wise fluctuations. Uses min-max optimization to synthesize worst-case bio-batch perturbations in representation space with strict angular geometric margin to preserve class discriminability. Introduces synergistic distribution alignment objective to prevent representation collapse during adversarial exploration.
...
Conclusion: ABRA effectively addresses biological batch effects in cell painting images through adversarial representation augmentation, achieving superior domain generalization for phenotypic profiling without requiring additional prior knowledge about treatments or cell culture information.
...
<details>
  <summary>Details</summary>
Motivation: BEV fusion detectors degrade under domain shift and sensor failures, limiting real-world reliability. Existing robustness approaches require architectural modifications or specialized retraining, making them difficult to integrate into deployed systems.

Method: Post Fusion Stabilizer (PFS) operates on intermediate BEV representations of existing detectors, producing refined feature maps for original detection heads. It stabilizes feature statistics under domain shift, suppresses spatial regions affected by sensor degradation, and adaptively restores weakened cues through residual correction. Designed as near-identity transformation to preserve performance while improving robustness.

Result: PFS achieves state-of-the-art results on nuScenes benchmark under diverse camera and LiDAR corruptions: +1.2% mAP improvement for camera dropout robustness and +4.4% mAP improvement for low-light performance, while maintaining lightweight footprint of only 3.3M parameters.

Conclusion: PFS provides an effective, lightweight solution for improving robustness of existing BEV fusion detectors against domain shifts and sensor failures without requiring architectural modifications or retraining, enabling easier integration into deployed autonomous driving systems.
...
TL;DR: CBM-Suite addresses four key limitations of Concept Bottleneck Models: lack of concept relevance metrics, linearity problem causing bypass of concept bottleneck, accuracy gap vs opaque models, and insufficient study of visual backbones/VLMs.


<details>
  <summary>Details</summary>
Motivation: Current Concept Bottleneck Models have fundamental limitations: no metric to pre-evaluate concept relevance, linearity problem causing models to bypass the concept bottleneck entirely, accuracy gap compared to opaque models, and lack of systematic study on how different visual backbones and vision-language models impact CBMs.

Method: Four-component framework: 1) Entropy-based metric to quantify concept set suitability; 2) Non-linear layer between concept activations and classifier to resolve linearity problem; 3) Distillation loss guided by linear teacher probe to narrow accuracy gap; 4) Comprehensive analysis of vision encoders, VLMs, and concept sets.

Result: CBM-Suite yields more accurate models and provides insights for improving concept-based interpretability through extensive evaluations.

Conclusion: CBM-Suite systematically addresses key limitations of Concept Bottleneck Models, improving both accuracy and interpretability while providing methodological tools for concept-based model development.
...
TL;DR: Proposes interpolated FID (iFID), a variant of reconstruction FID that strongly correlates with generation FID of latent diffusion models, addressing the poor correlation between rFID and gFID.


<details>
  <summary>Details</summary>
Motivation: The reconstruction FID (rFID) of VAEs shows poor correlation with generation FID (gFID) of latent diffusion models, creating a need for better evaluation metrics that can predict diffusion model performance.

Method: For each dataset element, retrieve its nearest neighbor in latent space, interpolate their latent representations, decode the interpolated latent, and compute FID between decoded samples and original dataset. Also refines understanding of rFID correlation with diffusion refinement phase quality versus iFID correlation with diffusion navigation phase quality.

Result: iFID achieves strong correlation with diffusion gFID (Pearson linear and Spearman rank correlations ~0.85), making it the first metric demonstrating such strong correlation. Provides explanation connecting to diffusion generalization and hallucination results.

Conclusion: iFID effectively bridges the gap between VAE reconstruction metrics and diffusion generation metrics, offering a predictive tool for diffusion model performance evaluation with strong empirical correlation.
...
### [9] [TempoSyncDiff: Distilled Temporally-Consistent Diffusion for Low-Latency Audio-Driven Talking Head Generation](https://arxiv.org/abs/2603.06057)
*Soumya Mazumdar,Vineet Kumar Rakesh*

Main category: cs.CV

TL;DR: TempoSyncDiff is a reference-conditioned latent diffusion framework for efficient audio-driven talking-head generation that uses teacher-student distillation to enable few-step inference while maintaining quality, with identity anchoring and temporal regularization to reduce flicker and identity drift.
...
Result: Experiments on LRS3 dataset show distilled diffusion models retain much of the reconstruction behavior of a stronger teacher while enabling substantially lower latency inference. Component-level metrics relative to VAE reconstructions and preliminary latency characterization (CPU-only and edge computing measurements) demonstrate feasibility for edge deployment.

Conclusion: Distilled diffusion models can achieve practical talking-head generation under constrained computational settings, representing an initial step toward efficient diffusion-based synthesis with improved temporal stability and audio-visual alignment.
...
TL;DR: IEC (Implicit Error Counting) addresses the reference-free RL problem by enumerating and scoring errors across task-relevant axes instead of comparing to ideal answers, showing strong performance in virtual try-on tasks.


<details>
  <summary>Details</summary>
Motivation: Existing RL methods like RLVR and RaR rely on ideal reference answers for rubric generation, but many real-world tasks lack single ideal answers and admit multiple valid outputs, creating a gap in post-training methods.

Method: IEC enumerates what responses get wrong rather than what they get right, applying severity-weighted scores across task-relevant axes and converting them into calibrated per-aspect rewards. Key design choices include implicit score emission and group calibration to stabilize optimization.

Result: IEC outperforms RaR on MDressBench across all metrics (CEC: 5.31 vs. 5.60 on flat references; 5.20 vs. 5.53 on non-flat). On VITON-HD and DressCode, IEC matches or surpasses six baselines on 6 of 8 perceptual metrics.
...
TL;DR: SemVID is a training-free visual token pruning framework for Video Temporal Grounding that preserves query-critical evidence and cross-frame connectivity while achieving 95.4% mIoU with only 12.5% tokens and 5.8x speedup.


<details>
  <summary>Details</summary>
Motivation: Video Temporal Grounding (VTG) requires processing long untrimmed videos with video-language models, which is computationally expensive. Existing token pruning methods designed for video QA degrade VTG performance because VTG depends on boundary-sensitive evidence and cross-frame reasoning chains.

Method: SemVID uses two VTG-specific pruning principles: Evidence Retention (ER) to keep query-critical patches around event boundaries, and Connectivity Strength (CS) to preserve token-level cross-frame connectivity. It allocates per-frame token budgets by balancing query relevance and inter-frame variation, then selects three token types: object tokens for query-critical evidence, motion tokens for transitions and cross-frame relays, and context tokens for scene continuity.

Result: Extensive experiments on VTG benchmarks show SemVID achieves strong accuracy-efficiency trade-off: retains up to 95.4% mIoU with only 12.5% visual tokens, delivers up to 5.8x prefill speedup, and consistently outperforms prior methods under same budgets.

Conclusion: SemVID effectively addresses the computational challenges of VTG by introducing principled, training-free token pruning that preserves both boundary-sensitive evidence and cross-frame connectivity, enabling efficient yet accurate temporal localization in videos.
...
TL;DR: OWL is a perception function that uses two visual motion cues (looming and rotation) to enable scaled 3D mapping and camera heading from raw image sequences without prior knowledge of scene or motion parameters.


<details>
  <summary>Details</summary>
Motivation: Address the complex challenges of 3D perception during motion by developing a unified analytical approach that can derive 3D information directly from visual motion cues without requiring explicit measurement of physical quantities like range and translation.

Method: OWL derives values from two fundamental visual motion cues: 1) perceived local visual looming of points near fixation, and 2) perceived rotation of rigid objects relative to fixation. It uses minimalistic, pixel-based parallel computations on raw visual motion image sequences without prior knowledge of stationary environments, moving objects, or camera motion.

Result: Simulations demonstrate that OWL achieves geometric constancy of 3D objects over time and enables scaled 3D scene reconstruction from visual motion cues alone. The function provides an alternative real-time representation for 3D points in relative motion.

Conclusion: OWL bridges theoretical concepts with practical applications in robotics and autonomous navigation, potentially serving as a building block for next-generation autonomous systems. It offers an alternative perspective on machine perception with implications extending to natural perception, behavioral psychology, and neural functionality.
...
TL;DR: MultiHaystack is a new benchmark for evaluating multimodal retrieval and reasoning across large, heterogeneous corpora, revealing that current MLLMs struggle with evidence retrieval despite strong reasoning when given relevant evidence.


<details>
  <summary>Details</summary>
Motivation: Existing MLLM benchmarks evaluate text, image, or video understanding separately but don't assess the critical real-world requirement of retrieving relevant evidence from large, heterogeneous multimodal corpora before reasoning. Most benchmarks restrict retrieval to small, single-modality candidate sets, oversimplifying the search space and overstating end-to-end reliability.

Method: Introduced MultiHaystack benchmark with over 46,000 multimodal retrieval candidates across documents, images, and videos, plus 747 open yet verifiable questions. Each question is grounded in a unique validated evidence item within the retrieval pool, requiring evidence localization across modalities and fine-grained reasoning.

Result: Models perform competitively when provided with corresponding evidence, but performance drops sharply when required to retrieve evidence from the full corpus. The strongest retriever (E5-V) achieves only 40.8% Recall@1. State-of-the-art MLLMs like GPT-5 experience significant accuracy drop from 80.86% (with evidence) to 51.4% (under top-5 retrieval).

Conclusion: Multimodal retrieval over heterogeneous pools remains a primary bottleneck for MLLMs. MultiHaystack serves as a valuable testbed that highlights limitations obscured by small-scale evaluations and promotes retrieval-centric advances in multimodal systems.
...
TL;DR: Audiovisual Geolocation framework combines interpretable acoustic perception with multimodal reasoning to achieve precise global geolocation, outperforming unimodal approaches.


<details>
  <summary>Details</summary>
Motivation: Multimodal Large Language Models have improved image-based localization, but precise global geolocation remains challenging due to visual ambiguity and underutilized auditory cues. The paper aims to resolve geographic ambiguity through interpretable perception and reasoning.

Method: Three-stage framework: (1) Perception stage uses mixture-autoregressive sparse autoencoder to decompose noisy audio into semantically grounded "acoustic atoms"; (2) Multimodal Reasoning stage employs MLLM finetuned via Group Relative Policy Optimization to synthesize acoustic atoms with visual features; (3) Precision Prediction stage uses Riemannian Flow Matching on the S² manifold.

Result: The framework significantly outperforms unimodal baselines. Experiments demonstrate that interpretable soundscape perception provides a critical orthogonal signal that, when coupled with multimodal reasoning, enables high-precision global localization.

Conclusion: Audiovisual geolocation through interpretable perception and reasoning effectively resolves geographic ambiguity, with soundscape analysis providing complementary information to visual cues for precise global localization.
...
### [15] [Any to Full: Prompting Depth Anything for Depth Completion in One Stage](https://arxiv.org/abs/2603.05711)
*Zhiyuan Zhou,Ruofeng Liu,Taichi Liu,Weijian Zuo,Shanshan Wang,Zhiqing Hong,Desheng Zhang*

Main category: cs.CV

TL;DR: Any2Full is a one-stage depth completion framework that adapts pretrained monocular depth estimation models using scale prompts to handle sparse depth inputs with varying patterns and sparsity levels.


<details>
  <summary>Details</summary>
Motivation: Existing RGBD-fused depth completion methods have limited domain generalization and robustness due to being jointly conditioned on specific training RGB distributions and depth patterns. Current two-stage approaches using monocular depth estimation models require explicit relative-to-metric alignment, adding computation and causing structured distortions.

Method: Any2Full reformulates depth completion as scale-prompting adaptation of a pretrained MDE model. It uses a Scale-Aware Prompt Encoder that distills scale cues from sparse depth inputs into unified scale prompts, guiding the MDE model toward globally scale-consistent predictions while preserving its geometric priors.

Result: Any2Full achieves superior robustness and efficiency: outperforms OMNI-DC by 32.2% in average AbsREL and delivers 1.4× speedup over PriorDA with the same MDE backbone. It establishes a new paradigm for universal depth completion.

Conclusion: Any2Full presents a one-stage, domain-general, pattern-agnostic framework that effectively leverages pretrained MDE models for universal depth completion through scale-prompting adaptation, addressing limitations of existing methods while maintaining efficiency.
...
TL;DR: Automated pipeline converts ImageNet training set to multi-label dataset using self-supervised Vision Transformers for unsupervised object discovery, improving model accuracy and transfer learning.


<details>
  <summary>Details</summary>
Motivation: ImageNet's single-label assumption creates label noise and limits learning signal since real images contain multiple objects; multi-label annotations better reflect real-world scenes but lack scalable training set solutions.

Method: Automated pipeline using self-supervised Vision Transformers for unsupervised object discovery, selecting regions aligned with original labels to train lightweight classifier, then applying it to all regions to generate coherent multi-label annotations.

Result: Multi-label supervision improves in-domain accuracy (up to +2.0 top-1 on ReaL, +1.5 on ImageNet-V2) and transferability (+4.2 mAP on COCO, +2.3 mAP on VOC); annotations align well with human judgment.

Conclusion: Accurate multi-label annotations enhance both classification performance and representation learning, demonstrating the importance of multi-label supervision for robust visual understanding.
...
### [18] [Full Dynamic Range Sky-Modelling For Image Based Lighting](https://arxiv.org/abs/2603.05758)
*Ian J. Maquignaz*
...
### [33] [PixARMesh: Autoregressive Mesh-Native Single-View Scene Reconstruction](https://arxiv.org/abs/2603.05888)
...
### [34] [InnoAds-Composer: Efficient Condition Composition for E-Commerce Poster Generation](https://arxiv.org/abs/2603.05898)
...
### [35] [Mitigating Bias in Concept Bottleneck Models for Fair and Interpretable Image Classification](https://arxiv.org/abs/2603.05899)
...
### [36] [CollabOD: Collaborative Multi-Backbone with Cross-scale Vision for UAV Small Object Detection](https://arxiv.org/abs/2603.05905)
...
### [39] [CORE-Seg: Reasoning-Driven Segmentation for Complex Lesions via Reinforcement Learning](https://arxiv.org/abs/2603.05911)
...
### [40] [BlackMirror: Black-Box Backdoor Detection for Text-to-Image Models via Instruction-Response Deviation](https://arxiv.org/abs/2603.05921)
...
### [59] [Technical Report: Automated Optical Inspection of Surgical Instruments](https://arxiv.org/abs/2603.05987)
...
### [63] [EffectMaker: Unifying Reasoning and Generation for Customized Visual Effect Creation](https://arxiv.org/abs/2603.06014)
...
### [71] [Devil is in Narrow Policy: Unleashing Exploration in Driving VLA Models](https://arxiv.org/abs/2603.06049)
...
### [150] [Koopman Regularized Deep Speech Disentanglement for Speaker Verification](https://arxiv.org/abs/2603.05577)
...
<details>
  <summary>Details</summary>
Motivation: Existing speaker verification systems often rely on labeled data, textual supervision, or large pretrained models, limiting scalability and practical deployment while raising sustainability concerns. There's a need for more efficient, unsupervised approaches that can effectively disentangle speaker characteristics from linguistic content.

Method: Deep Koopman Speech Disentanglement Autoencoder (DKSD-AE) combines a novel multi-step Koopman operator learning module with instance normalization. The Koopman operator models temporal dynamics while instance normalization helps disentangle speaker and content representations in an autoencoder framework.

Result: DKSD-AE achieves improved or competitive speaker verification performance compared to state-of-the-art baselines across multiple datasets while maintaining high content EER (confirming effective disentanglement). Results are obtained with substantially fewer parameters and without textual supervision, with stable performance under increased evaluation scale.

Conclusion: Koopman-based temporal modelling combined with instance normalization provides an efficient and principled solution for speaker-focused representation learning, offering better scalability and practical deployment potential while addressing sustainability concerns of existing approaches.
...
### [151] [Which Data Matter? Embedding-Based Data Selection for Speech Recognition](https://arxiv.org/abs/2603.05819)
...
TL;DR: Targeted data selection from large-scale heterogeneous speech data improves specialist ASR models by strategically choosing relevant subsets based on speaker, phonetic, and semantic embeddings, achieving up to 36.8% relative WER reduction with only 5% of data.


<details>
  <summary>Details</summary>
Motivation: Specialist ASR models targeting specific domains face challenges when trained on heterogeneous, multi-domain data: they lack capacity to learn from all available data, and there's a mismatch between training and test conditions that needs addressing.

Method: Use targeted data selection from 100k hours of in-the-wild training data. Represent speech samples using embeddings capturing complementary characteristics: speaker attributes, phonetic content, and semantic meaning. Analyze how relevance and diversity along these axes affect downstream ASR performance.

Result: Training on strategically selected 5% subsets can exceed performance of models trained on full dataset by up to 36.8% relative WER reduction on target domains, using CTC-based Conformer models.

Conclusion: Targeted data selection based on complementary speech characteristics (speaker, phonetic, semantic) is an effective strategy for optimizing specialist ASR models for specific domains, enabling superior performance with significantly less training data.
...
### [152] [How Well Do Current Speech Deepfake Detection Methods Generalize to the Real World?](https://arxiv.org/abs/2603.05852)
...
<details>
  <summary>Details</summary>
Motivation: Improved speech synthesis/voice conversion and social media compression/transmission mechanisms obscure deepfake artifacts, complicating reliable detection in real-world environments, creating need for representative evaluation benchmarks.

Method: Introduce ML-ITW dataset covering 14 languages, 7 major platforms, 180 public figures (28.39 hours total). Evaluate three detection paradigms: end-to-end neural models, self-supervised feature-based (SSL) methods, and audio large language models (Audio LLMs).
...
Conclusion: ML-ITW dataset addresses need for representative multilingual in-the-wild benchmark, revealing limitations of current detection methods and providing publicly available resource for advancing audio deepfake detection research.
...
### [153] [Do Compact SSL Backbones Matter for Audio Deepfake Detection? A Controlled Study with RAPTOR](https://arxiv.org/abs/2603.06164)
*Ajinkya Kulkarni,Sandipana Dowerah,Atharva Kulkarni,Tanel Alumäe,Mathew Magimai Doss*

Main category: cs.SD

TL;DR: RAPTOR framework demonstrates that multilingual HuBERT pre-training enables compact 100M parameter models to match larger commercial systems in audio deepfake detection, with better calibration under perturbation than WavLM variants.


<details>
  <summary>Details</summary>
Motivation: Most prior SSL-based audio deepfake detection research focuses on large wav2vec2-XLSR backbones, leaving compact models understudied. There's a need to understand how different SSL pre-training approaches affect cross-domain robustness and calibration in smaller models.

Method: RAPTOR framework conducts controlled study of compact SSL backbones (HuBERT and WavLM) within unified pairwise-gated fusion detector. Evaluates across 14 cross-domain benchmarks and introduces test-time augmentation protocol with perturbation-based aleatoric uncertainty to assess calibration.

Result: Multilingual HuBERT pre-training enables 100M parameter models to match larger commercial systems in cross-domain robustness. WavLM variants show overconfident miscalibration under perturbation, while iterative mHuBERT remains stable. SSL pre-training trajectory, not model scale, drives reliable detection.

Conclusion: SSL pre-training trajectory is more important than model scale for reliable audio deepfake detection. Multilingual HuBERT pre-training provides superior cross-domain robustness and calibration stability compared to WavLM variants in compact models.
...
### [154] [Whisper-CD: Accurate Long-Form Speech Recognition using Multi-Negative Contrastive Decoding](https://arxiv.org/abs/2603.06193)
*Hoseong Ahn,Jeongyun Chae,Yoonji Park,Kyuhong Shim*

Main category: cs.SD

TL;DR: Whisper-CD is a training-free contrastive decoding framework that reduces hallucinations and errors in long-form Whisper speech recognition by contrasting clean audio logits against acoustically perturbed negatives.


<details>
  <summary>Details</summary>
Motivation: Large encoder-decoder models like Whisper exhibit hallucinations, repetition loops, and content omissions in long-form speech recognition, with errors accumulating when previous segment transcriptions are used as context.

Method: Training-free contrastive decoding framework that contrasts clean-audio logits against negative logits from three acoustically motivated perturbations: Gaussian noise injection, silence signal, and audio temporal shift. Uses log-sum-exp operator to aggregate negatives into unified multi-negative objective for token-by-token decoding.
...
Conclusion: Whisper-CD operates purely at inference time, serving as a drop-in replacement for deployed Whisper systems without retraining, effectively addressing long-form recognition errors through acoustically grounded contrastive decoding.
...
### [155] [Prosodic Boundary-Aware Streaming Generation for LLM-Based TTS with Streaming Text Input](https://arxiv.org/abs/2603.06444)
*Changsong Liu,Tianrui Wang,Ye Ni,Yizhou Peng,Eng Siong Chng*

Main category: cs.SD

TL;DR: Proposes prosodic-boundary-aware post-training for streaming TTS to address unnatural prosody from missing lookahead and long-form collapse from unbounded context, using weakly time-aligned data and sliding-window inference.


<details>
  <summary>Details</summary>
Motivation: Streaming TTS with incremental text input faces two key challenges: unnatural prosody due to lack of future text lookahead, and long-form collapse caused by unbounded context accumulation during extended synthesis.

Method: Uses prosodic-boundary-aware post-training strategy adapting a pretrained LLM-based TTS model with weakly time-aligned data. The model learns early stopping at content boundaries with limited future text. During inference, a sliding-window prompt carries forward previous text and speech tokens to maintain bounded context and enable seamless concatenation.
...
Conclusion: Provides a robust solution for streaming TTS with incremental text by addressing both prosody and long-form stability issues through boundary-aware adaptation and bounded context management.
...
### [156] [RAMoEA-QA: Hierarchical Specialization for Robust Respiratory Audio Question Answering](https://arxiv.org/abs/2603.06542)
*Gaia A. Bertolino,Yuwei Zhang,Tong Xia,Domenico Talia,Cecilia Mascolo*

Main category: cs.SD

TL;DR: RAMoEA-QA is a hierarchically routed generative model for respiratory audio question answering that uses two-stage conditional specialization with Audio Mixture-of-Experts and Language Mixture-of-Adapters to handle diverse respiratory audio data and query intents.


<details>
  <summary>Details</summary>
Motivation: Current biomedical audio-language QA systems are monolithic and lack specialization mechanisms for diverse respiratory corpora and query intents. They are validated in limited settings and struggle with real-world heterogeneity in recordings (devices, environments, protocols) and diverse question formats/intents.

Method: RAMoEA-QA uses two-stage conditional specialization: 1) Audio Mixture-of-Experts routes each recording to suitable pre-trained audio encoder, 2) Language Mixture-of-Adapters selects LoRA adapter on shared frozen LLM to match query intent and answer format. This enables specialization of both acoustic representations and generation behavior per example.

Result: RAMoEA-QA consistently outperforms strong baselines and routing ablations with minimal parameter overhead, achieving in-domain test accuracy of 0.72 (vs. 0.61 and 0.67 for state-of-the-art baselines). It exhibits strongest generalization for diagnosis under domain, modality, and task shifts.

Conclusion: The hierarchical routing approach with conditional specialization effectively addresses heterogeneity challenges in respiratory audio QA, enabling unified handling of multiple question types and both discrete/continuous targets within a single multimodal system while maintaining strong generalization capabilities.
...
### [157] [Omni-C: Compressing Heterogeneous Modalities into a Single Dense Encoder](https://arxiv.org/abs/2603.05528)
*Kin Wai Lau,Yasar Abbas Ur Rehman,Lai-Man Po,Pedro Porto Buarque de Gusmão*

Main category: cs.MM

TL;DR: Omni-C is a single dense Transformer encoder that learns shared representations across images, audio, and text through unimodal contrastive pretraining, eliminating the need for multiple expert encoders or MoE routing while maintaining competitive performance.


<details>
  <summary>Details</summary>
Motivation: Current multimodal systems use separate expert modality encoders that scale linearly in complexity with added modalities, causing computational overhead. While unified Omni-models with Mixture-of-Expert architectures address this, they still inflate parameters and introduce routing overhead.

Method: Proposes Omni-C, a single dense Transformer-based encoder with maximum parameter sharing in the backbone and lightweight modality-specific projection heads. Uses unimodal contrastive pretraining on large-scale unaligned data without requiring MoE, paired supervision, or routing. Supports sequential modality processing and low-memory inference.

Result: Achieves performance comparable to expert models in unimodal and cross-modal tasks. Shows modest zero-shot degradation on audio and text that is largely recovered through lightweight linear probing or parameter efficient fine-tuning. Substantially reduces inference memory usage compared to multi-encoder baselines.

Conclusion: Omni-C advances efficient and scalable multimodal learning by providing a unified architecture that mitigates inter-modality conflicts without MoE routing, enabling deployment on memory-constrained systems while maintaining competitive performance.
...
### [158] [A Survey on Stacked Intelligent Metasurfaces: Fundamentals, Recent Advances, and Challenges](https://arxiv.org/abs/2603.05633)
*Chandan Kumar Sheemar,Wali Ullah Khan,Sourabh Solanki,George C. Alexandropoulos,Symeon Chatzinotas*

Main category: eess.SP

TL;DR: Survey on Stacked Intelligent Metasurfaces (SIMs) - multi-layer programmable metasurfaces for advanced electromagnetic processing in wireless communications, covering principles, modeling, hardware, applications, and future 6G directions.


<details>
  <summary>Details</summary>
Motivation: To provide comprehensive overview of SIMs as advanced programmable electromagnetic processors that enable richer wave-domain signal processing than conventional single-layer RIS, expanding design space for future 6G wireless systems.

Method: Survey methodology covering physical principles, modeling frameworks (cascaded operators, multiport impedance, network parameters), hardware realizations, architectural designs, and analysis of communication functionalities enabled by front-end metasurface processing.

Result: Comprehensive review of SIM technologies including: communication performance optimization, near-field/wideband transmission, learning-driven control, integrated sensing/communications, cell-free and non-terrestrial network architectures, and identification of open research problems.

Conclusion: SIMs represent promising direction for fully programmable electromagnetic processors in 6G systems, requiring further research in physical modeling, scalability, hardware-algorithm co-design, and network integration to realize their full potential.
...
### [159] [Frequency Response of Windowed DFT Phasor Estimation: Impact on Oscillation Observability](https://arxiv.org/abs/2603.05746)
*Jiahui Yang,Yuru Wu,Haozong Wang,Yu Liu,Biao Sun,Yilu Liu,Clifton Black*

Main category: eess.SP

TL;DR: This paper analyzes how windowed DFT phasor estimation in PMUs affects sub-synchronous oscillation observability, derives the complex-valued frequency response, and proposes a recovery method to restore true oscillation parameters.


<details>
  <summary>Details</summary>
Motivation: PMUs are widely used for sub-synchronous oscillation monitoring, but the effect of windowed DFT-based phasor estimation on oscillation observability is not fully characterized. There's a need to understand how the estimation window affects oscillation measurements and to provide guidance for industry practitioners.

Method: The authors derive the complete complex-valued frequency response of the windowed DFT phasor estimator under both magnitude and phase modulation. They analyze how the estimation window introduces frequency-dependent magnitude attenuation and phase shift to oscillation components, governed by the complex gain. A simple recovery method is proposed to restore the true oscillation amplitude and phase from PMU data using the analytically known complex gain.

Result: The analysis shows that windowed DFT phasor estimation introduces both frequency-dependent magnitude attenuation and phase shift to oscillation components. The proposed recovery method successfully restores true oscillation parameters. Results are validated through time-domain simulations, providing practical guidance for interpreting PMU-based oscillation measurements and selecting appropriate window lengths.

Conclusion: Windowed DFT phasor estimation in PMUs significantly affects oscillation observability through complex gain effects. The derived analytical framework and proposed recovery method enable accurate interpretation of PMU-based oscillation measurements, with practical implications for industry practitioners in selecting appropriate window lengths and properly analyzing oscillation data.
...
### [160] [Rethinking Next-Generation Signal Waveform: Integration of Orthogonality and Non-Orthogonality](https://arxiv.org/abs/2603.05752)
*Tongyang Xu,Shuangyang Li,Zhongxiang Wei,Gan Zheng,Izzat Darwazeh*

Main category: eess.SP

TL;DR: The paper explores sustainable physical layer waveform options for 6G, comparing SC-OFDM and SC-NOFS with 1D/2D precoding, finding SC-NOFS(2D) offers superior spectral efficiency, latency, and versatility for future 6G requirements.


<details>
  <summary>Details</summary>
Motivation: As 6G communications advance, there is increasing demand for new services and capabilities defined by ITU. A crucial challenge is developing signal waveforms that meet these demands while maintaining compatibility with existing standards, requiring a balanced approach that integrates non-orthogonality with orthogonality for both backward compatibility and forward innovation.

Method: The paper investigates two key signal formats: single-carrier orthogonal frequency division multiplexing (SC-OFDM) in 1D and 2D variants, and single-carrier non-orthogonal frequency shaping (SC-NOFS) in 1D and 2D variants. Both can utilize 1D frequency and 2D time-frequency precoding, offering enhanced frequency and time diversity, simplified processing, and resilience to delay-Doppler effects. The analysis focuses on comparative evaluation of these waveform options.

Result: SC-NOFS(2D) demonstrates significant advantages including improved spectral efficiency, reduced latency, enhanced frequency and time diversity, simplified processing, and resilience to delay-Doppler effects. The comparative analysis shows SC-NOFS(2D) provides a broader range of capabilities, particularly for applications requiring high data rate, high mobility, low-latency communication, sustainability, and interoperability.

Conclusion: SC-NOFS(2D) emerges as a strong candidate for future 6G applications, positioning it as a versatile solution for next-generation 6G communication that balances backward compatibility with forward innovation through its integration of non-orthogonality with orthogonality.
...
### [161] [In-Wave Computation Aided Stacked Intelligent Metasurfaces in Next-Generation Networks: Challenges and Opportunities](https://arxiv.org/abs/2603.05892)
*Mengbing Liu,Chau Yuen,Dusit Niyato,Bruno Clerckx,Lajos Hanzo*

Main category: eess.SP

TL;DR: Stacked intelligent metasurfaces (SIMs) enable in-wave computation during signal propagation through cascaded programmable layers, offering potential for reduced energy, lower latency, and improved joint communication-sensing-computation tasks.


<details>
  <summary>Details</summary>
Motivation: Traditional signal processing occurs solely after reception, limiting efficiency. SIMs aim to perform computation during signal propagation in the wave domain, expanding controllable degrees of freedom and enabling joint design of communication, sensing, and computation with potential benefits in energy efficiency, latency reduction, and task performance.

Method: The paper reviews state-of-the-art SIM research, including applications, functions, and characteristics. It demonstrates potential through case studies on neural-like analog inference and communication enhancement. The approach involves cascaded programmable metasurface layers that manipulate electromagnetic waves during propagation.

Result: SIM research is still at an early stage with challenges in scalability, controllability, nonlinearity, and robustness. However, case studies show promising potential for neural-like analog inference and communication enhancement. The technology represents a new signal processing paradigm for in-wave computation.

Conclusion: SIMs have the potential to establish a new signal processing paradigm for in-wave computation in next-generation networks, but significant challenges remain in scalability, controllability, nonlinearity, and robustness that require further research and development.
...
### [162] [Ill-Posedness Analysis of CSI-Based Electromagnetic Inverse Scattering for Material Reconstruction in ISAC Systems](https://arxiv.org/abs/2603.05918)
*Yubin Luo,Li Yu,Takumi Takahashi,Shaoyi Liu,Yuxiang Zhang,Jianhua Zhang,Hideki Ochiai*

Main category: eess.SP

TL;DR: The paper analyzes electromagnetic inverse scattering for material reconstruction in ISAC systems, identifies ill-conditioning origins in CSI-induced scattering operators, proposes ROI-constrained optimization with linear sampling for conditioning improvement, and validates with FDTD simulations.


<details>
  <summary>Details</summary>
Motivation: CSI-based electromagnetic inverse scattering enables physics-grounded material-aware digital twins in ISAC systems, but suffers from severe ill-conditioning of the scattering operator, requiring analysis of mathematical properties and operator structure to understand and mitigate this ill-posedness.

Method: Analyzes mathematical properties of electromagnetic inverse problem and ISAC scattering matrix structure; proves ROI restriction yields condition-number reduction; adopts ROI-constrained QP framework with linear sampling method for coarse ROI delineation and QP update in reduced subspace.

Result: Shows background-related matrix columns are highly coherent and dominate near rank deficiency, while scatterer-related columns are weakly correlated with coherence decreasing with probing frequencies; full-wave FDTD simulations validate pronounced conditioning improvement, substantial complexity savings, and improved robustness.

Conclusion: ROI-constrained optimization with linear sampling effectively mitigates ill-conditioning in electromagnetic inverse scattering for ISAC systems, providing provable condition-number reduction, tightened CRLB, and practical implementation benefits validated through comprehensive simulations.
...
### [163] [Channel Estimation for Reconfigurable Intelligent Surface Assisted Upper Mid-Band MIMO Systems](https://arxiv.org/abs/2603.06017)
*Jeongjae Lee,Chanwon Kim,Songnam Hong*

Main category: eess.SP

TL;DR: Proposes a conditioning-aware channel estimation framework for RIS-assisted UMB communications that addresses ill-conditioned LS problems via greedy column grouping and piecewise RIS phase design.


<details>
  <summary>Details</summary>
Motivation: RIS-assisted UMB communications face severe channel estimation challenges due to near-field propagation and transitional scattering, which induce strong spatial correlation and ill-conditioned LS formulations that degrade estimation performance.

Method: A conditioning-aware channel estimation framework that transforms the ill-conditioned high-dimensional problem into multiple well-conditioned subproblems via greedy column grouping. The method systematically separates highly correlated RIS elements into distinct sub-blocks through piecewise RIS phase design, improving Gram matrix conditioning and stabilizing piecewise LS reconstruction without relying on sparsity assumptions.

Result: Simulation results show the proposed method significantly outperforms conventional LS and OMP-based estimators in pilot-limited and transitional UMB regimes, achieving robust performance with low computational complexity.

Conclusion: The proposed conditioning-aware framework effectively addresses the channel estimation challenges in RIS-assisted UMB communications by transforming ill-conditioned problems into well-conditioned subproblems, offering a practical solution for 6G systems.
...
### [164] [Reinforcement Learning for Secrecy Optimization in Underwater Energy Harvesting Relay Network](https://arxiv.org/abs/2603.06046)
*Shalini Tripathi,Ankur Bansal,Chinmoy Kundu*

Main category: eess.SP

TL;DR: The paper proposes reinforcement learning-based optimal power allocation for secure underwater hybrid optical-acoustic relay networks with energy harvesting, addressing eavesdropping on acoustic links and optical link blockage.


<details>
  <summary>Details</summary>
Motivation: Secure communication in underwater energy-harvesting relay networks faces challenges: optical links (Gamma-Gamma turbulence with pointing errors) can be blocked by obstacles, while acoustic links are vulnerable to eavesdropping, requiring intelligent power allocation to maximize long-term secrecy performance.

Method: Formulate relay power allocation as infinite-horizon Markov decision process (MDP), propose model-based reinforcement learning-driven optimal power allocation (OPA) strategy, and develop lower-complexity alternatives: Greedy Algorithm (GA) and Naive Algorithm (NA).
...
Conclusion: Reinforcement learning provides effective power allocation for secure underwater hybrid optical-acoustic relay networks, balancing energy harvesting constraints with secrecy requirements against eavesdropping threats.
...
### [165] [A Retrieval-Assisted Framework for Wireless Localization](https://arxiv.org/abs/2603.06158)
*Haoyu Huang,Guangjin Pan,Kaixuan Huang,Shunqing Zhang,Yuhao Zhang,Musa Furkan Keskin,Zheng Xing,Henk Wymeersch*

Main category: eess.SP

TL;DR: Proposes a unified retrieval-assisted fingerprinting framework that integrates similarity-based and learning-based approaches for CSI-based wireless localization, using channel charting for efficient retrieval and graph attention networks for adaptive feature aggregation.


<details>
  <summary>Details</summary>
Motivation: Traditional similarity-based fingerprinting methods have high computational complexity and limited scalability in high-dimensional CSI spaces, while purely learning-based approaches fail to explicitly exploit correlations among reference fingerprints during inference. There's a need for a framework that combines the strengths of both paradigms.

Method: Proposes a unified retrieval-assisted fingerprinting localization framework that integrates similarity-based and learning-based paradigms. Uses channel charting to project high-dimensional CSI into low-dimensional latent space for efficient retrieval of locally correlated reference points. Then employs a graph attention network (GAT) to explicitly model inter-sample correlations between query CSI and retrieved references for adaptive, geometry-aware feature aggregation and position estimation.

Result: Extensive experiments on both real-world indoor and ray-tracing simulated outdoor scenarios demonstrate that the proposed method consistently outperforms state-of-the-art similarity-based and learning-based localization approaches.

Conclusion: The proposed unified framework successfully addresses limitations of both traditional similarity-based and purely learning-based approaches by combining efficient retrieval via channel charting with explicit correlation modeling through graph attention networks, achieving superior localization performance across diverse scenarios.
...
### [166] [MAD: A Multimodal and Multi-perspective Affective Dataset with Hierarchical Annotations](https://arxiv.org/abs/2603.06206)
...
<details>
  <summary>Details</summary>
Motivation: To create a comprehensive multimodal emotion dataset that enables observation of emotional dynamics from neural, physiological, and behavioral perspectives simultaneously, addressing the need for temporally aligned multimodal data in affective computing and neurophysiological modeling.

Method: Built through synchronous collection of diverse physiological signals (EEG, ECG, EOG, EMG, PPG, BCG) with tri-view RGB-D facial videos from 18 participants, incorporating a three-level emotion annotation framework spanning stimulus elicitation, subjective cognition, and behavioral expression.

Result: The dataset supports consistent and comparable performance across multiple benchmark experiments including intra-subject EEG emotion recognition, cross-subject EEG transfer learning, cardiac signal classification, multimodal physiological fusion, and multi-view facial emotion recognition.

Conclusion: MAD establishes a reliable benchmark for emotion recognition and cross-modal affective analysis, serving as a valuable resource for studying emotion mechanisms across multiple levels with its temporally aligned multimodal data and comprehensive annotation framework.
...
### [167] [Set-Prediction-Based J-Peak Detection for Pillow-Based Ballistocardiography](https://arxiv.org/abs/2603.06221)
*Shengwei Guo,Guobing Sun*

Main category: eess.SP

TL;DR: A set-prediction-based framework for BCG J-peak detection that directly models peaks as discrete events, outperforming segmentation-based approaches while reducing model complexity.


<details>
  <summary>Details</summary>
Motivation: Existing BCG J-peak detection methods use dense segmentation with heuristic post-processing, leading to redundant structures and sensitivity to parameters. There's a need for more direct and efficient peak detection approaches.

Method: Proposes a set-prediction-based J-peak detection framework that directly models peaks as discrete temporal events, eliminating segmentation heads and peak suppression. Uses a shared convolutional backbone and releases a pillow-based BCG-ECG dataset with multi-subject, multi-night sleep recordings and manually annotated J-peaks.

Result: The proposed method achieves superior detection performance compared to a U-Net-based segmentation baseline under the same convolutional backbone, while substantially reducing model parameters and computational complexity.

Conclusion: Event-level set prediction provides a concise and efficient modeling paradigm for BCG J-peak detection in sleep monitoring, offering better performance with reduced computational requirements.
...
### [168] [U6G XL-MIMO Radiomap Prediction: Multi-Config Dataset and Beam Map Approach](https://arxiv.org/abs/2603.06401)
*Xiaojie Li,Yu Han,Zhizheng Lu,Shi Jin,Chao-Kai Wen*

Main category: eess.SP

TL;DR: First XL-MIMO radiomap dataset for 6G systems with physics-informed beam maps enabling generalization to unseen array configurations without retraining.


<details>
  <summary>Details</summary>
Motivation: Existing radiomap prediction methods face two key challenges: 1) Data scarcity - current datasets only support small-scale arrays (up to 8×8) with isotropic antennas, far from the 1024-element directional arrays envisioned for 6G; 2) Generalization limitations - current methods encode array configurations as scalar parameters, forcing neural networks to extrapolate array-specific radiation patterns, which fails when predicting radiomaps for configurations absent from training data.

Method: Three-pronged approach: 1) Constructed first XL-MIMO radiomap dataset containing 78,400 radiomaps across 800 urban scenes, five frequency bands (1.8-6.7 GHz), and nine array configurations up to 32×32 uniform planar arrays with directional elements; 2) Established comprehensive benchmark framework covering practical scenarios; 3) Proposed beam maps - physics-informed spatial features that analytically compute array-specific coverage patterns, decoupling deterministic array radiation from data-learned multipath propagation.

Result: Beam maps integrated into existing architectures reduce mean absolute error by up to 60.0% when generalizing to unseen configurations and up to 50.5% when transferring to unseen environments. The complete dataset and code are publicly available.

Conclusion: The paper addresses both data scarcity and generalization limitations in XL-MIMO radiomap prediction through a comprehensive dataset, benchmark framework, and physics-informed beam map approach that enables generalization to arbitrary beam configurations without retraining by shifting generalization from neural network extrapolation to physics-based computation.
...
### [169] [The DCT Model as a Novel Regression Framework within a Lagrangian Formulation](https://arxiv.org/abs/2603.06418)
*Marc Martinez-Gost,Ana I. Perez Neira,Miguel Angel Lagunas*

Main category: eess.SP

TL;DR: A unified regression framework using Lagrangian formalism shows that polynomial and logistic regression share a common variational structure, with DCT-based models emerging as a novel and effective approach offering computational advantages over traditional methods.


<details>
  <summary>Details</summary>
Motivation: To establish a unified mathematical framework for regression methods using Lagrangian formalism, revealing common variational structures across different regression approaches and discovering new effective models like DCT-based regression.

Method: Develops a Lagrangian formalism-based regression framework where DCT (Discrete Cosine Transform) is used as constraints in the variational structure, leveraging the nearly orthogonal and bounded nature of cosine basis functions.

Result: The DCT-based model emerges naturally within this framework, offering computational advantages and improved convergence properties compared to traditional polynomial regression methods.

Conclusion: The DCT-based neuron shows strong potential as a powerful tool for regression analysis and related learning tasks within the unified Lagrangian framework.
...
### [170] [A Unified Multicarrier Waveform Framework for Next-generation Wireless Networks: Principles, Performance, and Challenges](https://arxiv.org/abs/2603.06515)
...
TL;DR: This paper proposes a unified multicarrier waveform framework for 6G networks, analyzing and comparing one-dimensional (OFDM, AFDM) and two-dimensional (OTFS) modulation waveforms to guide waveform selection for diverse channel conditions and applications.


<details>
  <summary>Details</summary>
Motivation: Next-generation wireless networks (6G and beyond) require enhanced flexibility, efficiency, and reliability in physical layer waveform design to address heterogeneous channel conditions and stringent quality-of-service demands, necessitating a systematic framework for waveform selection.

Method: The paper proposes a unified multicarrier waveform framework that: 1) examines design principles of state-of-the-art waveforms categorized into one-dimensional (OFDM, AFDM) and two-dimensional (OTFS) modulation; 2) studies their inherent resilience against channel-induced interference; 3) presents in-depth performance analysis comparing key performance indicators; and 4) explores their applications in various scenarios.

Result: The analysis reveals distinct suitability of different waveforms for diverse channel conditions, provides systematic characterization of waveform properties, and offers practical implementation guidelines for waveform selection in 6G networks.

Conclusion: This work serves as a pivotal reference for waveform adoption in future 6G standardization and network deployment by providing a comprehensive framework for evaluating and selecting appropriate multicarrier waveforms based on channel conditions and application requirements.
...
### [171] [Codebook Design and Baseband Precoding for Pragmatic Array-Fed RIS Hybrid Multiuser MIMO](https://arxiv.org/abs/2603.06541)
*Krishan Kumar Tiwari,Giuseppe Caire*

Main category: eess.SP

TL;DR: Extends hybrid digital-analog MIMO architecture with phase-only flat-top beam design for wide coverage, evaluates performance under realistic mmWave multipath channels, and proposes low-complexity framework compliant with 3GPP standards.


<details>
  <summary>Details</summary>
Motivation: Previous work introduced efficient HDA MU-MIMO architecture but only evaluated under pure LOS conditions. This work aims to extend the approach to realistic mmWave multipath environments while maintaining hardware simplicity and power efficiency, and ensuring compatibility with 3GPP standards.

Method: Proposes phase-only flat-top beam design for AMAF-RIS modules enabling wide angular coverage with low ripple/sidelobes; hierarchical beamforming codebooks; evaluates performance under realistic mmWave channels with LOS/NLOS components using 3D von Mises-Fisher distribution; implements low-complexity HDA MU-MIMO framework with user-beam association, dynamic user grouping, 3GPP-compliant channel estimation, and zero-forcing precoding with per-antenna power constraints.
...
Conclusion: The extended HDA MU-MIMO architecture successfully operates in realistic mmWave multipath environments, maintains hardware efficiency, and demonstrates practical implementation compatibility with existing 3GPP standards while achieving good performance metrics.
...
### [172] [LiveSense: A Real-Time Wi-Fi Sensing Platform for Range-Doppler on COTS Laptop](https://arxiv.org/abs/2603.06545)
*Jessica Sanson,Rahul C. Shah,Maximilian Pinaroc,Cagri Tanriover,Valerio Frascolla*

Main category: eess.SP

TL;DR: LiveSense transforms COTS Wi-Fi NICs into centimeter-level Range-Doppler sensors while maintaining communication, using Intel AX211/BE201 chips to extract synchronized CSI at ≥40Hz with real-time processing and GUI visualization.


<details>
  <summary>Details</summary>
Motivation: There's a need for accurate sensing capabilities using existing commercial Wi-Fi hardware without sacrificing communication functionality, enabling applications like human detection, respiration monitoring, and gesture recognition using standard devices.

Method: Uses COTS Intel Wi-Fi 6E/7 NICs (AX211/BE201) with custom software to extract synchronized CSI at ≥40Hz, performs time-phase alignment and self-interference cancellation on-device, and provides real-time data streams to a Python/Qt GUI for range-Doppler processing.

Result: Achieves centimeter-level range sensing despite limited 160MHz bandwidth, can detect distance/velocity of nearby attendees, monitor respiration (micro-motion), and recognize hand gestures in real-time.

Conclusion: LiveSense demonstrates the first accurate range sensing from commercial Wi-Fi with limited bandwidth, transforming standard laptops into dual-purpose communication-sensing platforms with practical applications in human-computer interaction and monitoring.
...
### [173] [Activation Steering for Accent Adaptation in Speech Foundation Models](https://arxiv.org/abs/2603.05813)
*Jinuo Sun,Yang Xiao,Sung Kyun Chung,Qiuchi Hu,Gongping Huang,Eun-Jung Holden,Ting Dang*

Main category: eess.AS

TL;DR: The paper identifies accent information as concentrated in middle encoder layers of ASR models and introduces parameter-free accent steering to reduce word error rates across eight accents.
...
Method: The authors treat accent variation as an interpretable subspace in hidden representations, extract layer-wise encoder activations, estimate mean-shift directions capturing accent-induced representation shifts, and analyze layer-wise accent sensitivity. They then introduce parameter-free accent steering that modifies representations during inference without updating model weights.

Result: Analysis reveals that accent information concentrates in a narrow band of middle encoder layers. Experiments across eight accents show consistent word error rate reductions using the proposed parameter-free accent steering approach.

Conclusion: Accent information can be identified as a structured subspace in ASR model representations, concentrated in middle encoder layers, and can be effectively controlled through parameter-free steering during inference to improve recognition accuracy across diverse accents.
...
### [174] [ImKWS: Test-Time Adaptation for Keyword Spotting with Class Imbalance](https://arxiv.org/abs/2603.05821)
...
<details>
  <summary>Details</summary>
Motivation: Keyword spotting accuracy degrades in noisy environments, and standard adaptation requires original/labeled audio. Test time adaptation uses unlabeled test audio but fails to handle severe class imbalance between rare keywords and frequent background sounds, causing standard entropy minimization to become overconfident and biased toward the frequent background class.
...
Conclusion: ImKWS effectively addresses the class imbalance problem in test time adaptation for keyword spotting by redesigning entropy minimization with separate reward/penalty branches and transformation consistency, enabling robust performance in noisy, imbalanced environments without requiring labeled data.
...
### [175] [Reconstruct! Don't Encode: Self-Supervised Representation Reconstruction Loss for High-Intelligibility and Low-Latency Streaming Neural Audio Codec](https://arxiv.org/abs/2603.05887)
...
TL;DR: JHCodec improves neural audio codec performance using self-supervised representation reconstruction (SSRR) loss, achieving better intelligibility, faster convergence, and zero-lookahead streaming capability with reduced training costs.
...
Method: Proposes self-supervised representation reconstruction (SSRR) loss for codec training, which reconstructs distilled self-supervised representations from codec outputs. This approach enables zero-lookahead architecture for streaming Transformer-based codecs.

Result: SSRR significantly accelerates convergence (competitive results with single GPU), enhances intelligibility, enables high intelligibility without additional lookahead in streaming codecs, and achieves state-of-the-art performance with minimal latency and reduced training cost.

Conclusion: JHCodec demonstrates that SSRR loss fundamentally improves codec training and performance, offering a practical solution for real-time deployment with open-sourced implementation, training pipeline, and demo.
...
### [176] [Activation Steering for Accent-Neutralized Zero-Shot Text-To-Speech](https://arxiv.org/abs/2603.05977)
*Mu Yang,John H. L. Hansen*

Main category: eess.AS

TL;DR: A training-free, post-hoc method using inference-time activation steering to neutralize accents in zero-shot TTS while preserving speaker timbre, by applying pre-extracted steering vectors derived from activation differences between accented and native speech.


<details>
  <summary>Details</summary>
Motivation: Zero-shot TTS models struggle to disentangle voice timbre and accent, often inheriting both from reference speakers. There's a need for accent-neutralization while preserving original speaker timbre without requiring model retraining.

Method: Extract layer-specific "steering vectors" offline from internal activation differences between accented and native speech within the TTS model. During inference, apply these vectors to guide the model toward accent-neutralized, timbre-preserving speech generation.

Result: Empirical results show the steering vectors effectively mitigate output accent and exhibit strong generalizability to unseen accented speakers, providing a practical solution for accent-free voice cloning.

Conclusion: The proposed inference-time activation steering approach offers a novel, training-free method for accent neutralization in zero-shot TTS while preserving speaker timbre, demonstrating practical utility for accent-free voice cloning applications.
...
### [177] [StreamVoiceAnon+: Emotion-Preserving Streaming Speaker Anonymization via Frame-Level Acoustic Distillation](https://arxiv.org/abs/2603.06079)
*Nikita Kuzmin,Kong Aik Lee,Eng Siong Chng*

Main category: eess.AS

TL;DR: Proposes a streaming speaker anonymization method that preserves emotional content through supervised finetuning with neutral-emotion utterance pairs and emotion distillation on acoustic tokens, achieving significant emotion preservation improvements with minimal computational overhead.


<details>
  <summary>Details</summary>
Motivation: Current neural audio codec language models for streaming speaker anonymization degrade emotional content because content tokens discard emotional information and models default to dominant acoustic patterns rather than preserving paralinguistic attributes.

Method: Supervised finetuning with neutral-emotion utterance pairs from the same speaker, combined with frame-level emotion distillation on acoustic token hidden states. All modifications are confined to finetuning, taking less than 2 hours on 4 GPUs with zero inference latency overhead.
...
Conclusion: The proposed finetuning approach effectively preserves emotional content in streaming speaker anonymization with minimal computational cost, demonstrating significant improvements in emotion preservation while maintaining intelligibility and privacy.
...
### [178] [Continual Adaptation for Pacific Indigenous Speech Recognition](https://arxiv.org/abs/2603.06310)
*Yang Xiao,Aso Mahmudi,Nick Thieberger,Eliathamby Ambikairajah,Eun-Jung Holden,Ting Dang*

Main category: eess.AS

TL;DR: Adapting speech foundation models to low-resource Pacific Indigenous languages reveals severe representational drift and catastrophic forgetting, highlighting the need for robust adaptation strategies.


<details>
  <summary>Details</summary>
Motivation: Speech foundation models struggle with low-resource Pacific Indigenous languages due to severe data scarcity, and full fine-tuning risks catastrophic forgetting, creating a need for effective adaptation methods.

Method: Empirical study adapting models to real-world Pacific datasets, evaluating strategies including Full Fine-Tuning and Low-Rank Adaptation (LoRA), and analyzing a continual learning framework for sequential language acquisition.

Result: Adapting to distant Pacific languages causes severe internal representational drift, creating a strict plasticity-stability dilemma. LoRA adapts well initially but suffers from catastrophic forgetting during sequential learning.

Conclusion: This study highlights the urgent need for robust adaptation strategies tailored to underrepresented languages, particularly for Pacific Indigenous languages facing severe data scarcity.
...
### [179] [Classification of Autistic and Non-Autistic Children's Speech: A Cross-Linguistic Study in Finnish, French, and Slovak](https://arxiv.org/abs/2603.06327)
*Sofoklis Kakouros,Ida-Lotta Myllylä*

Main category: eess.AS

TL;DR: Cross-linguistic study shows autism-related speech cues partially generalize across Finnish, French, and Slovak, with Finnish performing best (Accuracy 0.84) and French showing poor cross-language transfer (F1 0.42).


<details>
  <summary>Details</summary>
Motivation: To investigate whether acoustic-prosodic speech cues for autism detection are language-specific or language-general across typologically distinct languages (Finnish, French, Slovak), using classification as an analytical tool rather than seeking state-of-the-art performance.

Method: Combined supervised classification with within-language and cross-corpus transfer experiments using acoustic-prosodic features. Implemented speaker-level classification benchmarks with cross-validation, pooled corpora models, and leave-one-corpus-out experiments to test generalization across languages.

Result: Within-language models: Finnish best (Accuracy 0.84, F1 0.88), Slovak (Accuracy 0.63, F1 0.68), French (Accuracy 0.68, F1 0.56). Cross-language: Pooled model Accuracy 0.61, F1 0.68. Cross-transfer: Good to Slovak (F1 0.70) and Finnish (F1 0.78), poor to French (F1 0.42). Feature analysis showed partially shared but not fully language-invariant acoustic markers.
...
### [180] [Cross-linguistic Prosodic Analysis of Autistic and Non-autistic Child Speech in Finnish, French and Slovak](https://arxiv.org/abs/2603.06332)
*Ida-Lotta Myllylä,Sofoklis Kakouros*

Main category: eess.AS

TL;DR: Autistic speakers across Finnish, French, and Slovak show distinct prosodic profiles including increased intensity variability, clearer voice quality, reduced temporal intensity dynamics, and lower fundamental frequency, with both cross-linguistic patterns and language-specific nuances.


<details>
  <summary>Details</summary>
Motivation: While prosodic differences in autism are well-documented, there is limited cross-linguistic evidence. The study aims to investigate prosody in autism across multiple languages to identify potential language-independent markers and move beyond deficiency-based models.

Method: Analyzed multilingual corpus of Finnish, French, and Slovak speakers. Extracted 88 acoustic features from over 5,000 inter-pausal units. Used Principal Component Analysis (PCA) for data reduction and Linear Mixed-Effects Models (LMMs) for statistical analysis.

Result: Cross-linguistically, autistic speakers showed: increased general intensity variability; clearer, less breathy voice quality (higher Harmonics-to-Noise Ratio and alpha ratio); reduced temporal intensity dynamics; lower central f0. Language-specific patterns: Slovak aligned with cross-linguistic f0 patterns but diverged on voice quality; Finnish mirrored broader voice quality findings.

Conclusion: Voice quality and intensity dynamics should be included alongside traditional pitch measures when studying potential language-independent markers of autism. Findings support a complex, acoustically distinct prosodic profile across languages rather than deficiency-based models.
...
### [181] [Doctor or Patient? Synergizing Diarization and ASR for Code-Switched Hinglish Medical Conditions Extraction](https://arxiv.org/abs/2603.06373)
...
TL;DR: A robust system for extracting medical conditions from code-switched Hinglish clinical dialogues using EEND-VC for speaker diarization, fine-tuned Qwen3 ASR for transcription, and LLMs for condition extraction, achieving top performance in DISPLACE-M challenge.
...
Method: 1) EEND-VC (End-to-End Neural Diarization with Vector Clustering) for accurate speaker diarization in dense, overlapped Doctor-Patient Conversations; 2) Domain-specific fine-tuning of Qwen3 ASR model with Devanagari script normalization and dialogue-level LLM error correction for transcription; 3) Benchmarking open and proprietary LLMs for medical condition extraction using both text-based cascade and multimodal End-to-End audio frameworks.

Result: Achieved 18.59% tcpWER (turn-coupled word error rate) for transcription; First place out of 25 participants in the DISPLACE-M challenge; While proprietary E2E models set the performance ceiling, the open cascaded architecture proved highly competitive.

Conclusion: The proposed robust system effectively addresses challenges in code-switched clinical dialogue processing through advanced diarization, domain-adapted ASR, and LLM-based extraction, demonstrating state-of-the-art performance on real-world Hinglish medical conversations with publicly released implementations.
